...
export default class DatCp {
	...
	async upload(paths) {
		...
		for (const path of paths) await this.uploadPath(path, '')
		this.printTotal()
	}
	...
	async ensurePathValid(path) {
		...
		if (stats.isDirectory()) return await this.ensureDirValid(path, stats)
	...
	async ensureDirValid(path, stats) {
		if (path[path.length - 1] !== '/') this.countPath(path, stats)
		const dirPaths = await fs.readdir(path)
		for (const dirPath of dirPaths) await this.ensurePathValid(nodePath.join(path, dirPath))
	}

	async uploadPath(path, datPath) {
		const stats = await fs.lstat(path)
		if (stats.isFile()) await this.uploadFile(path, datPath)
		else if (stats.isDirectory()) await this.uploadDir(path, datPath)
	}
	...
	async uploadDir(path, datPath) {
		...
		const dirPaths = await fs.readdir(path)
		for (const dirPath of dirPaths) await this.uploadPath(nodePath.join(path, dirPath), datPath)
	}
	...
	async downloadPath(path) {
		...
		if (stats.isDirectory()) await this.downloadDir(path, stats)
		else await this.downloadFile(path, stats)
	}
	...
	async downloadDir(path, stats) {
		...
		for (const dirPath of dirPaths) await this.downloadPath(nodePath.join(path, dirPath))
	}
	...
	countPath(path, stats) {
		...
		if (!this.options.dryRun) return
		if (stats.isFile()) logger.info(`${formatSize(stats.size).padEnd(8)} ${path}`)
		else if (stats.isDirectory()) logger.info(`${'-'.padEnd(8)} ${path}`)
	}
